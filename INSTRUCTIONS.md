...
[`src/components/DynamicForm.jsx`](./src/components/DynamicForm.jsx).
...
| `save()`                                     | function       | `=> Promise<boolean>`. Validates; returns `false` if there are errors.                        |
...
// No instanceId => isNewRecord === true; save() creates the record.
```
...
2. **Validation errors** (`errors`) — the raw `getValidationErrors()` response. It is
   **nested**, not a flat map. Top-level fields appear under `_root`, and child-table
   errors are keyed by table → row → field:
    ...
    `DynamicForm.jsx` passes each field its own `errors[fieldId]`).
...
1. Study the reference component: [`src/form/index.jsx`](./src/components/DynamicForm.jsx).
